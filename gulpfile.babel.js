...
const paths = {
  ...
  reports: {
    basePath: process.env.CIRCLE_TEST_REPORTS || './reports'
  },
  karma: 'karma.conf.js',
  dist: 'dist'
};
...
let mocha = lazypipe()
  .pipe(plugins.mocha, {
    reporter: process.env.CIRCLECI ? 'mocha-junit-reporter' : 'spec',
    reporterOptions: {mochaFile: `${paths.reports.basePath}/junit/results.xml`},
    timeout: 5000,
    require: [
      './mocha.conf'
    ]
